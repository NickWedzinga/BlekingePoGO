...
async def add_to_leaderboards_test(bot, ctx):
    ...
    assert (len(execute_statement(create_select_query(table_name="test", where_key="name", where_value=f"'{ctx.author.display_name}'")).all(as_dict=True)) == 0)
    try:
        ...
        assert (len(execute_statement(create_select_query(table_name="test", where_key="name", where_value=f"'{ctx.author.display_name}'")).all(as_dict=True)) == 1)
        # remove user from leaderboard table again
        execute_statement(create_delete_query(table_name="test", where_key="name", where_value=f"'{ctx.author.display_name}'"))
